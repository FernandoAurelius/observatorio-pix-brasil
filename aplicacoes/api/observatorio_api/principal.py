...
from collections import defaultdict as DicionarioPadrao, deque as Fila
from pathlib import Path as Caminho
from threading import Lock as Trava
from fastapi import FastAPI as AplicacaoHTTP, Request as Requisicao, HTTPException as ExcecaoHTTP
from fastapi.responses import JSONResponse as RespostaJSON, Response as Resposta
...
from .catalogo import NUMERICAS,CATEGORICAS,FONTE
from .esquemas import Filtros,Analise,ConsultaTabela,ConsultaPanorama,ConsultaRegressao,ConsultaDistribuicao,ConsultaTCL,ConsultaLGN
from .provedor_bcb import ProvedorBCB,FalhaFonte
from . import servicos
from nucleo_estatistico import ajustar_distribuicao,simular_grandes_numeros,simular_limite_central

aplicacao = AplicacaoHTTP(docs_url='/api/documentacao',openapi_url='/api/openapi.json',redoc_url=None,title='Observatório Pix Brasil',version='1.0.0',description='API educacional: dados BCB + núcleo matemático próprio. Não afiliado ao BCB.')
origens_cors = [origem.strip() for origem in sistema.getenv(
    'ORIGENS_CORS', 'http://localhost:3000,http://127.0.0.1:3000').split(',') if origem.strip()]
aplicacao.add_middleware(IntermediarioCORS, allow_origins=origens_cors,
    ...
    if requisicao.url.path.startswith('/api/') and requisicao.url.path != '/api/saude':
        ...
        with trava_acessos:
            ...
            while fila and instante-fila[0]>60:
                fila.popleft()
            if len(fila)>=120:
                return RespostaJSON(status_code=429,content={'mensagem':'Muitas consultas. Aguarde um minuto.'},headers={'Retry-After':'60'})
...
async def tratar_fonte(requisicao: Requisicao, erro: FalhaFonte):
    ...
    return RespostaJSON(status_code=503,content={'mensagem':str(erro),'tipo':'fonte_indisponivel'})
...
async def tratar_matematica(requisicao: Requisicao, erro: ValueError):
    ...
    return RespostaJSON(status_code=422,content={'mensagem':str(erro),'tipo':'dominio_invalido'})
...
async def tratar_validacao(requisicao: Requisicao, erro: ErroValidacaoRequisicao):
    ...
    return RespostaJSON(status_code=422,content={'mensagem':'Revise os parâmetros, seus tipos e limites: '+', '.join(campos),'tipo':'parametros_invalidos'})
...
def consultar_catalogo():
    ...
    return {'numericas':NUMERICAS,'categoricas':CATEGORICAS,'fonte':FONTE,'modo':provedor.modo,
        'periodicidade':'mensal','unidade_observacional':'município-mês',
...
def consultar_filtros(consulta: Filtros):
    ...
    municipios = {}
    for linha in todos:
        if (not consulta.regiao or linha['regiao']==consulta.regiao) and (not consulta.uf or linha['uf']==consulta.uf):
            municipios[linha['codigo_municipio']] = {'codigo':linha['codigo_municipio'],'nome':linha['municipio'],'uf':linha['uf']}
    return empacotar(conjunto,registros,{'regioes':sorted({linha['regiao'] for linha in todos}),
        'estados':sorted({linha['uf'] for linha in todos if not consulta.regiao or linha['regiao']==consulta.regiao}),
...
def consultar_dados(consulta: ConsultaTabela):
    ...
    conjunto,registros = carregar(consulta)
    if consulta.busca:
        registros = [linha for linha in registros if consulta.busca.casefold() in (linha['municipio']+' '+linha['uf']).casefold()]
    ...
    ordenados = sorted(preenchidos,key=lambda linha:linha[consulta.ordenar],reverse=consulta.decrescente)+nulos
    inicio = (consulta.pagina-1)*consulta.tamanho
    return empacotar(conjunto,registros,{'linhas':ordenados[inicio:inicio+consulta.tamanho],
        'total':len(ordenados),'pagina':consulta.pagina,'paginas':max(1,(len(ordenados)+consulta.tamanho-1)//consulta.tamanho)})
...
def exportar_dados(consulta: Filtros):
    ...
    conjunto,registros = carregar(consulta)
    colunas = ['competencia','codigo_municipio','municipio','uf','estado','regiao']+list(NUMERICAS)
    saida = fluxos.StringIO()
    escritor = arquivo_csv.DictWriter(saida,fieldnames=colunas,delimiter=';',lineterminator='\n')
    escritor.writeheader()
    for linha in registros:
        segura = {campo:("'"+str(valor) if isinstance(valor,str) and valor.startswith(('=','+','-','@','\t','\r')) else valor) for campo,valor in linha.items()}
        escritor.writerow(segura)
    return Resposta(content=('﻿'+saida.getvalue()).encode('utf-8'),media_type='text/csv; charset=utf-8',
        headers={'Content-Disposition':f'attachment; filename="pix_{consulta.inicio}_{consulta.fim}.csv"'})
...
def consultar_lgn(consulta: ConsultaLGN):
    ...
    return simular_grandes_numeros(consulta.repeticoes,consulta.experimento,consulta.resultado,consulta.semente)
...
def consultar_tcl(consulta: ConsultaTCL):
    ...
    conjunto,registros = carregar(consulta)
    valores,ausentes = servicos.extrair_numeros(registros,consulta.variavel)
    resultado = simular_limite_central(valores,consulta.tamanho_amostra,consulta.repeticoes,consulta.semente)
...
def consultar_validacao():
    ...
    if not caminho.is_file():
        return {'executada':False,'mensagem':'Execute python scripts/validar_nucleo.py para gerar esta evidência.'}
...
def exportar_relatorio(consulta: Filtros):
    ...
    from .relatorios import preparar_evidencias, escrever_resumo
    conjunto,registros = carregar(consulta)
    evidencias = preparar_evidencias(conjunto,registros,consulta)
    return Resposta(content=escrever_resumo(evidencias).encode('utf-8'),media_type='text/markdown; charset=utf-8',
        headers={'Content-Disposition':f'attachment; filename="descobertas_{consulta.inicio}_{consulta.fim}.md"'})
