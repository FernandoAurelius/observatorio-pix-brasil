...
from nucleo_estatistico import (soma,media,mediana,resumo,percentil,histograma,detectar_atipicos,
    frequencias_categoricas,regressao_linear,predizer,ajustar_distribuicao,simular_limite_central)
from .catalogo import NUMERICAS,CATEGORICAS

def filtrar(conjunto: dict, filtros) -> list[dict]:
    """Filtra sem modificar o cache; categorias são comparadas exatamente."""
    return [linha for linha in conjunto['registros']
...
def contexto(conjunto: dict, registros: list[dict]) -> dict:
    ...
    return {'modo':conjunto['modo'],'fonte':conjunto['fonte'],'inicio':conjunto['inicio'],'fim':conjunto['fim'],
        'registros':len(registros),'municipios':len({linha['codigo_municipio'] for linha in registros}),
        'unidade_observacional':'município-mês','proveniencia':conjunto['proveniencia'],
...
def analisar_descritiva(registros: list[dict], consulta) -> dict:
    ...
    if consulta.variavel in CATEGORICAS:
        ...
        return {'tipo':'categorica','frequencias':frequencias,'quantidade':len(registros),
    ...
    caixa.pop('indices')
    return {'tipo':'numerica','medidas':resumo(valores,consulta.amostral),
            'frequencias':histograma(valores,consulta.classes),'caixa':caixa,
            'percentil':{'percentual':consulta.percentual,'valor':percentil(valores,consulta.percentual)},
            'ausentes_excluidos':ausentes,'variavel':NUMERICAS[consulta.variavel]}
...
def agregar_fluxo(registros: list[dict], papel: str, pessoa: str) -> dict:
    ...
    completos = [linha for linha in registros if all(linha[campo] is not None for campo in campos_valor+campos_quantidade)]
    ...
    return {'valor':valor,'quantidade':quantidade,'ticket_medio':dividir_seguro(valor,quantidade),
...
def panorama(registros: list[dict], consulta) -> dict:
    ...
    def agrupar(campo):
        ...
        return grupos
    meses = [{'competencia':chave,**agregar_fluxo(linhas,consulta.papel,consulta.pessoa)} for chave,linhas in sorted(agrupar('competencia').items())]
    regioes = [{'regiao':chave,**agregar_fluxo(linhas,consulta.papel,consulta.pessoa)} for chave,linhas in agrupar('regiao').items()]
    municipios = [{'codigo_municipio':chave,'municipio':linhas[0]['municipio'],'uf':linhas[0]['uf'],
                    **agregar_fluxo(linhas,consulta.papel,consulta.pessoa)} for chave,linhas in agrupar('codigo_municipio').items()]
    ...
    concentracao = dividir_seguro(soma(linha['valor'] for linha in municipios[:maiores]),total['valor'])
    return {'totais':total,'meses':meses,'regioes':sorted(regioes,key=lambda linha:linha['valor'],reverse=True),
        'municipios':municipios,'pessoas':[{'pessoa':tipo.upper(),**agregar_fluxo(registros,consulta.papel,tipo)} for tipo in ('pf','pj')],
        'concentracao':{'quantidade':maiores,'participacao':concentracao},
        'papel':consulta.papel,'pessoa':consulta.pessoa,
...
def analisar_regressao(registros: list[dict], consulta) -> dict:
    ...
    completos = [linha for linha in registros if linha[consulta.variavel_x] is not None and linha[consulta.variavel_y] is not None]
    ...
    if len(indices)>2000:
        extremos = {min(indices,key=lambda indice:valores_x[indice]),max(indices,key=lambda indice:valores_x[indice]),
                    min(indices,key=lambda indice:valores_y[indice]),max(indices,key=lambda indice:valores_y[indice])}
        indices = sorted(extremos | set(aleatoriedade.Random(42).sample([indice for indice in indices if indice not in extremos],2000-len(extremos))))
    pontos = [{'valor_x':valores_x[indice],'valor_y':valores_y[indice],
               'residuo':modelo['residuos'][indice],'previsto':modelo['previstos'][indice],
               'municipio':completos[indice]['municipio'],'uf':completos[indice]['uf'],'competencia':completos[indice]['competencia']}
              for indice in indices]
    modelo.pop('residuos'); modelo.pop('previstos')
    predicao = None
    if consulta.valor_predicao is not None:
        predicao = {'valor_x':consulta.valor_predicao,'valor_y':predizer(consulta.valor_predicao,modelo['intercepto'],modelo['inclinacao']),
                    'extrapolacao':not modelo['minimo_x']<=consulta.valor_predicao<=modelo['maximo_x']}
    return {'modelo':modelo,'pontos':pontos,'ausentes_excluidos':len(registros)-len(completos),
        'reta':[{'valor_x':valor,'valor_y':predizer(valor,modelo['intercepto'],modelo['inclinacao'])} for valor in (modelo['minimo_x'],modelo['maximo_x'])],
        'predicao':predicao,'amostragem_visual':len(indices)<len(completos),
...
def descobertas(registros: list[dict], filtros) -> list[dict]:
    ...
    try:
        ...
        texto_ajuste = f"r={ajuste['pearson']:.6f}; R²={ajuste['r_quadrado']:.6f}; n={ajuste['quantidade']}." if ajuste['pearson'] is not None else 'Resposta constante: r e R² indefinidos.'
    ...
    return [
        {'titulo':'Concentração municipal do valor pago',
         'texto':f"Os {concentracao['quantidade']} municípios com maior valor pago concentram {100*participacao:.2f}% do valor do recorte." if participacao is not None else 'Sem valor positivo no recorte.',
         'evidencia':concentracao,'grafico':'/','ressalva':'Soma PF+PJ somente no lado pagador. Não mede riqueza nem renda por habitante.'},
        {'titulo':'Média e mediana das transações pagas por PF',
         ...
         'evidencia':{'media':centro,'mediana':mediano,'razao':dividir_seguro(centro,mediano),'n':len(valores)},
         'grafico':'/descritiva','ressalva':'Comparar com o histograma e com a assimetria; diferença entre média e mediana isoladamente não comprova um formato.'},
        {'titulo':'Associação entre quantidade e valor pago por PF','texto':texto_ajuste,
         'evidencia':ajuste,'grafico':'/regressao',
