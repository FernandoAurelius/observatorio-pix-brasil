...
def listar_competencias(inicio: str, fim: str) -> list[str]:
    ...
    atual = DataHora.now(FusoHorario.utc).strftime('%Y%m')
...
def normalizar_linhas(linhas: list[dict], competencia: str) -> tuple[list[dict],dict]:
    ...
    obrigatorios = {'AnoMes','Municipio_Ibge','Municipio','Estado_Ibge','Estado','Regiao'} | {dados['original'] for dados in NUMERICAS.values()}
    for linha in linhas:
        faltantes = obrigatorios-set(linha)
        if faltantes:
            raise FalhaFonte('Esquema da fonte mudou. Campos ausentes: '+', '.join(sorted(faltantes)))
        ...
        if chave in vistos:
            raise FalhaFonte('A fonte devolveu chave município-mês duplicada. Nenhuma soma automática foi realizada.')
        vistos.add(chave)
        estado_codigo = str(linha['Estado_Ibge']).removesuffix('.0')
        registro = {'competencia':periodo,'codigo_municipio':codigo,
            'municipio':str(linha['Municipio']).strip(),'estado':str(linha['Estado']).strip(),
            'uf':UNIDADES_FEDERATIVAS.get(estado_codigo,estado_codigo),'regiao':str(linha['Regiao']).strip().title()}
    ...
    return resultado, {'recebidos':len(linhas),'aceitos':len(resultado),'identificadores_excluidos':excluidos,'ausentes':ausentes}
...
class ProvedorBCB:
    ...
    def obter_mes(proprio, competencia: str) -> dict:
        ...
        with trava_mes:
            ...
            endereco = (f"{ENDERECO_BCB}?@DataBase='{competencia}'&$format=json"
                f"&$filter=AnoMes%20eq%20{competencia}&$orderby=Municipio_Ibge")
            if not registros:
                raise FalhaFonte(f'Não há registros municipais válidos para {competencia}; o mês pode não estar publicado.')
            resumo_digital = resumos_digitais.sha256(serializacao.dumps(registros,sort_keys=True,ensure_ascii=False).encode()).hexdigest()
            conjunto = {'registros':registros,'proveniencia':{'competencia':competencia,'modo':proprio.modo,
                'url':endereco,'consultado_em':DataHora.now(FusoHorario.utc).isoformat(),
                'sha256_normalizado':resumo_digital,'qualidade':qualidade}}
            with proprio.trava:
    ...
    def baixar_mes(proprio, competencia: str) -> list[dict]:
        ...
        """
        with cliente_http.Client(timeout=45,transport=proprio.transporte,follow_redirects=False) as cliente:
            ...
            endereco_consulta = (f"{ENDERECO_BCB}?@DataBase='{competencia}'&$format=json"
                f"&$filter=AnoMes%20eq%20{competencia}&$orderby=Municipio_Ibge")
            resposta = None
            for tentativa in range(3):
                try:
                    ...
                    if tentativa == 2 or (resposta is not None and resposta.status_code not in (429,500,502,503,504)):
                        raise FalhaFonte('Não foi possível consultar o Banco Central. Verifique acesso à internet e disponibilidade da API.') from erro
                    relogio.sleep(0.5*(2**tentativa))
            try:
                dados = resposta.json()
                linhas = dados['value']
            ...
            if dados.get('@odata.nextLink'):
                raise FalhaFonte('A fonte passou a paginar a resposta. Atualize o cliente antes de usar dados parciais.')
            return linhas
    ...
    def obter_periodo(proprio, inicio: str, fim: str) -> dict:
        ...
        return {'registros':[linha for conjunto in conjuntos for linha in conjunto['registros']],
                'proveniencia':[conjunto['proveniencia'] for conjunto in conjuntos],
                'modo':proprio.modo,'fonte':FONTE,'inicio':inicio,'fim':fim}
