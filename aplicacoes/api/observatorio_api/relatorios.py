...
from .esquemas import ConsultaPanorama, Analise, ConsultaRegressao
from nucleo_estatistico import ajustar_distribuicao
...
def preparar_evidencias(conjunto: dict, registros: list[dict], filtros) -> dict:
    ...
    return {'gerado_em':DataHora.now(FusoHorario.utc).isoformat(),
        'contexto':servicos.contexto(conjunto,registros),'filtros':parametros,
...
def escrever_resumo(evidencias: dict) -> str:
    ...
    linhas = ['# Observatório Pix Brasil - resultados do recorte','',
    ...
    linhas += ['## Medidas e evidências','',
        ...
        '## Proveniência','', '```json',serializacao.dumps(contexto,ensure_ascii=False,indent=2),'```','',
