...
def auditar(inicio: str, fim: str) -> dict:
    ...
    negativos = {campo: sum(linha[campo] is not None and linha[campo] < 0 for linha in registros)
                 for campo in NUMERICAS}
    discretas = [campo for campo, definicao in NUMERICAS.items() if definicao['tipo'] == 'discreta']
    tipos_incorretos = {
        campo: sum(linha[campo] is not None and (
            isinstance(linha[campo], bool) or
            (campo in discretas and not isinstance(linha[campo], int)) or
            (campo not in discretas and not isinstance(linha[campo], float)))
            for linha in registros)
        for campo in NUMERICAS
    }
    resumo = {
        'consultado_em': DataHora.now(FusoHorario.utc).isoformat(),
        'fonte': conjunto['fonte'], 'inicio': inicio, 'fim': fim,
        ...
        'competencias_duplicadas': len(conjunto['proveniencia']) - len({
            item['competencia'] for item in conjunto['proveniencia']}),
        'regra_limpeza': ('Excluem-se somente linhas sem código IBGE municipal válido de sete dígitos; '
    ...
    resumo['atende_requisitos'] = (
        resumo['registros'] >= 1000 and resumo['variaveis_numericas'] >= 4 and
        resumo['variaveis_categoricas'] >= 2 and resumo['duplicidades_municipio_mes'] == 0 and
        resumo['tipos_incorretos_total'] == 0 and resumo['negativos_total'] == 0 and
        resumo['competencias'] == [f'2025{mes:02d}' for mes in range(1, 13)]
    )
    return resumo
...
def imprimir_tabela(resumo: dict) -> None:
    ...
    linhas = [
        ('Registros', resumo['registros']), ('Municípios', resumo['municipios']),
        ('Estados/UFs', resumo['estados']), ('Regiões', resumo['regioes']),
