...
from observatorio_api.catalogo import NUMERICAS,CATEGORICAS
...
if __name__ == '__main__':
    ...
    preenchidos = {campo:sum(linha[campo] is not None for linha in registros) for campo in NUMERICAS}
    relatorio = {'proveniencia':conjunto['proveniencia'],'registros':len(registros),'categorias_distintas':categorias,
        'numericas_preenchidas':preenchidos,'numero_variaveis_numericas':len(NUMERICAS),
        'apto_volume_esquema':len(registros)>=1000 and len(NUMERICAS)>=4 and sum(valor>=2 for valor in categorias.values())>=2,
    ...
    destino = RAIZ/'documentacao/evidencias/fonte_bcb.json';destino.parent.mkdir(exist_ok=True,parents=True)
    destino.write_text(serializacao.dumps(relatorio,ensure_ascii=False,indent=2),encoding='utf-8')
