...
def gerar_figuras(evidencias: dict, destino: Caminho):
    ...
    gravar_figura(
        destino / "concentracao.svg", "Maiores valores pagos (PF + PJ; lado pagador)", elementos
    )
    ...
    gravar_figura(
        destino / "histograma.svg", "Distribuicao da quantidade de Pix pagos por PF", elementos
    )
    regressao = evidencias["regressao"]
    pontos = regressao["pontos"]
    reta = regressao["reta"]
    menor_x = min(ponto["valor_x"] for ponto in pontos)
    maior_x = max(ponto["valor_x"] for ponto in pontos)
    menor_y = min(ponto["valor_y"] for ponto in pontos + reta)
    maior_y = max(ponto["valor_y"] for ponto in pontos + reta)
    ...
    elementos = "".join(
        f'<circle cx="{coordenada_x(ponto["valor_x"])}" cy="{coordenada_y(ponto["valor_y"])}" r="2" fill="#0d5c46" opacity=".35"/>'
        for ponto in pontos
    )
    ...
    gravar_figura(
        destino / "regressao.svg",
        "Quantidade x valor pago por PF; reta de minimos quadrados",
        elementos,
    )
