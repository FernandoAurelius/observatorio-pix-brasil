    ...
    for pasta in [raiz/'pacotes',raiz/'aplicacoes/api']:
        for caminho in pasta.rglob('*.py'):
            ...
            for no in sintaxe.walk(arvore):
                if isinstance(no,sintaxe.Import):modulos=[apelido.name.split('.')[0] for apelido in no.names]
                elif isinstance(no,sintaxe.ImportFrom):modulos=[(no.module or '').split('.')[0]]
                else:continue
...
def teste_docstrings_e_nomes():
    for nome,funcao in inspecao.getmembers(nucleo,inspecao.isfunction):
        ...
        assert nome not in {'mean','median','mode','std','variance','covariance','correlation','predict'}
