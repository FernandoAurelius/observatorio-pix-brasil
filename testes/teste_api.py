...
from observatorio_api.esquemas import ConsultaRegressao,ConsultaPanorama

@testes.mark.parametrize('rota,corpo',[
 ('panorama',{}),('dados',{}),('descritiva',{}),('descritiva',{'variavel':'regiao'}),
 ('regressao',{'valor_predicao':100}),('distribuicoes',{'distribuicao':'normal'}),
 ('distribuicoes',{'distribuicao':'exponencial'}),('distribuicoes',{'distribuicao':'uniforme'}),
 ('distribuicoes',{'distribuicao':'poisson'}),('simulacoes/tcl',{'repeticoes':50}),
 ('descobertas',{}),('filtros',{})])
...
def teste_cors_local_restrito(cliente):
    resposta = cliente.options('/api/catalogo', headers={
        'Origin': 'http://localhost:3000',
        'Access-Control-Request-Method': 'GET',
    })
    assert resposta.status_code == 200
    assert resposta.headers['access-control-allow-origin'] == 'http://localhost:3000'
    resposta_bloqueada = cliente.options('/api/catalogo', headers={
...
def teste_validacao_entradas(cliente):
    for corpo in [{'classes':101},{'inicio':'abc'},{'variavel':'SELECT *'},{'percentual':-2}]:
    ...
    assert cliente.post('/api/simulacoes/tcl',json={'tamanho_amostra':1000,'repeticoes':10000}).status_code==422
