...
from observatorio_api.catalogo import FONTE, NUMERICAS
...
def gerar_amostra_teste(competencia: str = "202512") -> list[dict]:
    ...
    for indice in range(1200):
        ...
        linha = {
            ...
            "codigo_municipio": f"99{indice:05d}",
            "municipio": f"Município de teste {indice + 1:04d}",
            "estado": estado,
            "uf": uf,
            "regiao": regiao,
        }
        for papel in ("pagador", "recebedor"):
            for pessoa in ("pf", "pj"):
                ...
                valor_medio = (
                    gerador.uniform(80, 600) if pessoa == "pf" else gerador.uniform(800, 4500)
                )
                linha[f"quantidade_{papel}_{pessoa}"] = quantidade
                linha[f"valor_{papel}_{pessoa}"] = round(quantidade * valor_medio, 2)
                linha[f"pessoas_{papel}_{pessoa}"] = max(
                    1, round(quantidade / gerador.uniform(5, 35))
                )
...
class ProvedorTeste:
    ...
    def obter_periodo(self, inicio: str, fim: str) -> dict:
        ...
        qualidade = {
            ...
            "aceitos": len(registros),
            "identificadores_excluidos": 0,
            ...
            "proveniencia": [
                {
                    ...
                    "modo": "bcb",
                    "url": "teste local",
                    "consultado_em": "2026-09-20T00:00:00-03:00",
                    "sha256_normalizado": "0" * 64,
                    "qualidade": qualidade,
                }
            ],
            "modo": "bcb",
