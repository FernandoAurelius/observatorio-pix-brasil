...
CONJUNTOS = [
    ...
    [-20, -2, 0, 3, 12],
    [0.1, 0.2, 0.3, 0.4],
    [1000000000000 + indice * 0.5 for indice in range(100)],
...
def teste_medidas_basicas(valores):
    ...
    comparar(
        quartis(valores), referencia_numerica.percentile(valores, [25, 50, 75], method="linear")
    )
...
def teste_dispersao(valores, amostral):
    ...
    if referencia_numerica.mean(valores) != 0:
        comparar(
            coeficiente_variacao(valores, amostral),
            100 * abs(referencia_estatistica.variation(valores, ddof=int(amostral))),
        )
...
def teste_percentis(percentual, valores):
    comparar(
        percentil(valores, percentual),
        referencia_numerica.percentile(valores, percentual, method="linear"),
    )
...
def teste_covariancia(amostral):
    ...
    comparar(
        covariancia(valores_x, valores_y, amostral),
        referencia_numerica.cov(valores_x, valores_y, ddof=int(amostral))[0, 1],
    )
...
def teste_api_academica_com_nomes_explicitos():
    ...
    comparar(
        calcular_coeficiente_variacao(valores),
        100 * referencia_numerica.std(valores, ddof=1) / abs(referencia_numerica.mean(valores)),
    )
    comparar(
        calcular_covariancia(valores, respostas), referencia_numerica.cov(valores, respostas)[0, 1]
    )
    comparar(
        calcular_correlacao_pearson(valores, respostas),
...
def teste_regressao_pearson_determinacao(semente):
    ...
    comparar(
        correlacao_pearson(valores_x, valores_y),
        referencia_numerica.corrcoef(valores_x, valores_y)[0, 1],
    )
    ...
    comparar(
        predizer(150, modelo["intercepto"], modelo["inclinacao"]),
        referencia.intercept + 150 * referencia.slope,
    )
...
def teste_histograma(classes, valores):
    ...
    comparar(
        sum(linha["densidade"] * (linha["superior"] - linha["inferior"]) for linha in linhas), 1
    )
...
def teste_atipicos():
    ...
    esperado = [
        ...
        if valor < primeiro - 1.5 * intervalo or valor > terceiro + 1.5 * intervalo
    ]
...
def teste_categoricas():
    ...
    assert {linha["categoria"]: linha["frequencia"] for linha in obtido} == dict(
        zip(categorias, contagens, strict=False)
    )
...
def teste_densidades(valor):
    ...
    comparar(
        densidade_uniforme(valor, -1, 5), referencia_estatistica.uniform.pdf(valor, loc=-1, scale=6)
    )
...
def teste_poisson_numerica(intensidade):
    for valor in [0, 1, 2, round(intensidade), round(intensidade + matematica.sqrt(intensidade))]:
        comparar(
            massa_poisson(valor, intensidade),
            referencia_estatistica.poisson.pmf(valor, intensidade),
        )
...
def teste_estimacao(familia):
    ...
    if familia == "normal":
        ...
        comparar(parametros["media"], centro)
        comparar(parametros["desvio_populacional"], desvio)
    ...
    if familia == "uniforme":
        ...
        comparar(parametros["inferior"], localizacao)
        comparar(parametros["superior"], localizacao + escala)
    if familia == "poisson":
        comparar(parametros["intensidade"], referencia_numerica.mean(valores))
...
def teste_casos_limite():
    ...
    with testes.raises(ValueError):
        correlacao_pearson([1, 1], [2, 3])
    with testes.raises(ValueError):
        regressao_linear([1, 1], [2, 3])
    with testes.raises(ValueError):
        covariancia([1, 2], [2, 3, 4])
    with testes.raises(ValueError):
        percentil([1, 2], 101)
    with testes.raises(ValueError):
        percentil([1, 2], -1)
    with testes.raises(ValueError):
        densidade_normal(0, 0, 0)
    with testes.raises(ValueError):
        densidade_uniforme(0, 2, 2)
    with testes.raises(ValueError):
        densidade_exponencial(0, -1)
    with testes.raises(ValueError):
        massa_poisson(0, -1)
    with testes.raises(ValueError):
        ajustar_distribuicao([0.5, 1.2], "poisson")
    with testes.raises(ValueError):
        ajustar_distribuicao([-1, 2], "exponencial")
    with testes.raises(ValueError):
        ajustar_distribuicao([1, 2], "inexistente")
    with testes.raises(ValueError):
        histograma([1, 2], 0)
...
def teste_lgn_reprodutivel():
    ...
    with testes.raises(ValueError):
        simular_grandes_numeros(10, "moeda", 6)
...
def teste_tcl_reprodutivel_e_erro_padrao():
    ...
    with testes.raises(ValueError):
        simular_limite_central(valores, 1000, 10000)
