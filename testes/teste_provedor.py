...
from observatorio_api.provedor_bcb import ProvedorBCB,FalhaFonte,normalizar_linhas,listar_competencias

def linha_oficial(codigo='5300108'):
    """Cria uma linha sintética no esquema externo, não dado real."""
    return {'AnoMes':'202512','Municipio_Ibge':codigo,'Municipio':'Teste','Estado_Ibge':53,
            'Estado':'Distrito Federal','Regiao':'Centro-Oeste',
            **{definicao['original']:10 for definicao in NUMERICAS.values()}}


def teste_normalizacao():
    linha = linha_oficial();linha['VL_PagadorPF']=12345.67;linha['QT_PagadorPJ']=None
...
def teste_qualidade():
    with testes.raises(FalhaFonte):normalizar_linhas([{}],'202512')
    with testes.raises(FalhaFonte):normalizar_linhas([linha_oficial(),linha_oficial()],'202512')
    with testes.raises(FalhaFonte):normalizar_linhas([linha_oficial()],'202501')
    linha=linha_oficial();linha['VL_PagadorPF']=-1
    with testes.raises(FalhaFonte):normalizar_linhas([linha],'202512')
...
def teste_consulta_filtrada_e_cache():
    ...
    primeiro=provedor.obter_mes('202512');segundo=provedor.obter_mes('202512')
...
def teste_paginacao_inesperada():
    ...
    provedor=ProvedorBCB(cliente_http.MockTransport(lambda requisicao:cliente_http.Response(200,json=resposta)))
    with testes.raises(FalhaFonte,match='paginar'):provedor.obter_mes('202512')


def teste_falha_da_fonte_e_explicita():
    provedor=ProvedorBCB(cliente_http.MockTransport(lambda requisicao:cliente_http.Response(403)))
    with testes.raises(FalhaFonte):provedor.obter_mes('202512')
...
def teste_janela():
    ...
    for inicio,fim in [('202500','202501'),('202502','202501'),('202401','202512'),('200001','200012')]:
        with testes.raises(ValueError):listar_competencias(inicio,fim)
