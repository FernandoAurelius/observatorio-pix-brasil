...
from .descritiva import media, desvio_padrao, variancia, histograma
...
def densidade_normal(valor: float, centro: float, desvio: float) -> float:
    ...
    return matematica.exp(-0.5*((valor-centro)/desvio)**2)/(desvio*matematica.sqrt(2*matematica.pi))
...
def massa_poisson(valor: float, intensidade: float) -> float:
    ...
    return min(1.0, matematica.exp(-intensidade+valor*matematica.log(intensidade)-matematica.lgamma(valor+1)))
...
def ajustar_distribuicao(valores, distribuicao: str, classes: int = 25) -> dict:
    ...
    avisos = ['Inspeção visual não comprova uma distribuição. Não foram calculados p-valores de aderência.',
    ...
    if distribuicao == 'normal':
        ...
        funcao = lambda numero: densidade_normal(numero,centro,desvio)
        ...
        funcao = lambda numero: densidade_exponencial(numero,1/centro)
    elif distribuicao == 'uniforme':
        densidade_uniforme(menor,menor,maior)
        parametros = {'inferior':menor,'superior':maior}
        funcao = lambda numero: densidade_uniforme(numero,menor,maior)
        ...
        observados = [{'valor':numero,'probabilidade':contagens[numero]/len(numeros)} for numero in ordenados[::passo]]
        ...
        curva = [{'valor':numero,'densidade':massa_poisson(numero,centro)} for numero in range(inicio,fim+1,passo_curva)]
        if passo > 1:
            avisos.append(f'Gráfico exibe 1 a cada {passo} valores distintos, sem renormalizar probabilidades; cálculos usam todos os dados.')
        avisos.append('A curva exibe suporte próximo de λ. Zeros visuais em outras faixas podem decorrer de probabilidades extremamente pequenas.')
        return {'tipo':'discreta','distribuicao':distribuicao,'parametros':{'intensidade':centro},
                'dispersao_media':variancia(numeros,False)/centro if centro else None,
                'histograma':histograma(numeros,classes),'observados':observados,'curva':curva,'avisos':avisos}
    else:
        raise ValueError('Distribuição desconhecida.')
    amplitude_dados = maior-menor
    curva = [{'valor':menor+amplitude_dados*indice/240,'densidade':funcao(menor+amplitude_dados*indice/240)} for indice in range(241)]
    return {'tipo':'continua','distribuicao':distribuicao,'parametros':parametros,
            'histograma':histograma(numeros,classes),'curva':curva,'avisos':avisos}
