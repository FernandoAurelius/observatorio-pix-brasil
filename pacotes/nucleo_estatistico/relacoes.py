...
def covariancia(valores_x, valores_y, amostral: bool = True) -> float:
    ...
    return matematica.fsum((valor_x-centro_x)*(valor_y-centro_y)
        for valor_x,valor_y in zip(numeros_x,numeros_y)) / (len(numeros_x)-int(amostral))
...
def correlacao_pearson(valores_x, valores_y) -> float:
    ...
    soma_xy = matematica.fsum((valor_x-centro_x)*(valor_y-centro_y)
                            for valor_x,valor_y in zip(numeros_x,numeros_y))
...
def coeficiente_determinacao(observados, previstos) -> float:
    ...
    residuos = matematica.fsum((real-estimado)**2 for real,estimado in zip(reais,estimados))
...
def regressao_linear(valores_x, valores_y) -> dict:
    ...
    soma_xy = matematica.fsum((numero_x-centro_x)*(numero_y-centro_y)
                            for numero_x,numero_y in zip(numeros_x,numeros_y))
    ...
    return {'inclinacao':inclinacao, 'intercepto':intercepto,
        ...
        'covariancia':covariancia(numeros_x,numeros_y), 'previstos':previstos,
        'residuos':[real-previsto for real,previsto in zip(numeros_y,previstos)],
        'minimo_x':min(numeros_x), 'maximo_x':max(numeros_x), 'quantidade':len(numeros_x),
