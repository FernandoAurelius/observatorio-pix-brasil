...
from .descritiva import media, desvio_padrao, histograma
from .distribuicoes import densidade_normal

def simular_grandes_numeros(repeticoes: int = 2000, experimento: str = 'moeda', resultado: int = 1, semente: int = 42) -> dict:
    ...
    if isinstance(repeticoes,bool) or not isinstance(repeticoes,int) or not 10 <= repeticoes <= 100000:
        raise ValueError('Use de 10 a 100000 repetições inteiras.')
    faces = 2 if experimento == 'moeda' else 6
    if isinstance(resultado,bool) or not isinstance(resultado,int) or resultado not in range(1,faces+1):
    ...
    return {'pontos':pontos,'probabilidade':1/faces,'frequencia_final':acertos/repeticoes,
            'acertos':acertos,'repeticoes':repeticoes,'semente':semente,
            'aviso':'A frequência tende à probabilidade em muitas repetições; o erro pode aumentar temporariamente.'}

def simular_limite_central(valores, tamanho_amostra: int = 30, repeticoes: int = 1000, semente: int = 42) -> dict:
    ...
    if isinstance(tamanho_amostra,bool) or not isinstance(tamanho_amostra,int) or not 2 <= tamanho_amostra <= 1000:
        raise ValueError('O tamanho da amostra deve estar entre 2 e 1000.')
    if isinstance(repeticoes,bool) or not isinstance(repeticoes,int) or not 50 <= repeticoes <= 10000:
    ...
    curva = [] if erro_padrao == 0 else [
        {'valor':menor+(maior-menor)*indice/200,'densidade':densidade_normal(menor+(maior-menor)*indice/200,centro,erro_padrao)}
        for indice in range(201)]
    return {'media_populacao':centro,'desvio_populacao':desvio,'media_das_medias':media(medias),
        'desvio_das_medias':desvio_padrao(medias,True),'erro_padrao':erro_padrao,
        'histograma_original':histograma(numeros,30),'histograma_medias':histograma(medias,30),
        'curva':curva,'medias':medias,'tamanho_amostra':tamanho_amostra,
        'repeticoes':repeticoes,'semente':semente,'com_reposicao':True,
